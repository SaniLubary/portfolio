...
export default function ExperienceSlider({ children, rightSide = false }: { children: ReactElement[] | ReactElement, rightSide: boolean }) {
  const [currentSlide, setCurrentSlide] = useState(0)

  const currentSlideRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    gsap.to(currentSlideRef.current, { opacity: '100%', duration: 0.3, x: '0%', ease: 'power3.inOut'})
  }, [currentSlide])
  ...
  const handleLeftArrowClick = () => {
    ...
    gsap.timeline()
      .to(currentSlideRef.current, { opacity: 0, duration: 0.3, x: '100%', ease: 'power3.in' })
  ...
  return (
    <div className={`${!rightSide? 'dissapearLeft':'dissapearRight'} flex flex-col h-40 items-center`}>
      <div>
          ...
          <div className="h-fit overflow-hidden rounded-lg">
            ...
            <div className="flex justify-center mt-4">
              <div className="w-1/2 flex justify-around">
                {Array.isArray(children)
                  && children.map((el, i) => {
                    return <div key={el.props.alt + i}
                      onClick={() => currentSlide !== i && ((currentSlide < i) ? handleRightArrowClick() : handleLeftArrowClick())}
                      className={`w-4 h-4 cursor-pointer border-gray-200 border-2 rounded-full ${i === currentSlide && 'bg-slate-100'}`}>
                    </div>
                  }
                  )}
              </div>
